...
export default function CartCheckoutModal({ onClose, onSuccess }: Props) {
  ...
  const inputCls = (field: keyof CustomerForm) =>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Order summary */}
        <div className="bg-zinc-50 mx-6 mt-5 px-4 py-3 space-y-1.5">
          {items.map(({ product, quantity }) => (
            <div key={product.id} className="flex items-center justify-between text-sm">
              <span className="text-zinc-600">
                {product.name} <span className="text-zinc-400">× {quantity}</span>
              </span>
          ...
          <div className="flex items-center justify-between pt-1.5 mt-1.5 border-t border-zinc-200">
